...
public class UserServiceImpl implements UserService {
    ...
    private static final String temp_name = "微商大学";
    private static final Integer temp_level = -1;
...
//            return new H5LoginResultVo(null, null, null , accessTokenVo.getErrcode() , accessTokenVo.getErrmsg());
    ...
    public Integer isSubscribe(String merchantId, String userId) {
        ...
        log.info("是否已关注过公众号：" + subscribe);
        return subscribe;
    }

    @Override
    public List<ExcelVo> getAllList() {
...
//        private int cardNum = 0;
        ...
        List<User> allList = userMapper.getAllList();
        List<ExcelVo> resultList = new ArrayList<>();
        int i = 0;
        for (User user : allList) {
            if (user.getMemberLevel() == null || user.getMemberLevel() < 4 || user.getRegisterMobile() == null) {
                continue;
            }
            log.info("行数" + user.getUserId());
            ExcelVo excelVo = new ExcelVo();
            excelVo.setRegisterFrom(user.getRegisterFrom() == 1 ? "注册" : "导入");
            excelVo.setRegisterPhone(user.getRegisterMobile());
            excelVo.setMemberLevel(user.getMemberLevel());
            excelVo.setLevelName(user.getLevelName());
            excelVo.setRegisterTime(user.getRegisterTime());
            if (user.getGender() == null) {
                excelVo.setGender("未设置");
            } else {
                excelVo.setGender(user.getGender() == 0 ? "男" : "女");
            }
            excelVo.setRealName(user.getRealName() == null ? "未设置" : user.getRealName());
            Integer amount = orderService.getConsumeAmountByPhoneAndMid(user.getRegisterMobile(), "1509688041");
            excelVo.setConsumeAmount(amount == null ? 0 : amount / 100);
            String referrer = user.getReferrer();

            if (referrer == null) {
                excelVo.setFinalRefPhone(temp_name);
                excelVo.setFinalRefName(temp_name);
                excelVo.setFinalRefMemberLevel(temp_level);
                excelVo.setFinalRefLevelName(temp_name);
                excelVo.setRecommendPhone(temp_name);
                excelVo.setRecommendName(temp_name);
                excelVo.setRecommendMemberLevel(temp_level);
                excelVo.setRecommendLevelName(temp_name);
            } else {
                User refUser = userMapper.getUserByUserId(referrer);
                if (refUser == null) {
                    excelVo.setFinalRefPhone(temp_name);
                    excelVo.setFinalRefName(temp_name);
                    excelVo.setFinalRefMemberLevel(temp_level);
                    ...
                    excelVo.setRefPhone(refUser.getRegisterMobile() == null ? "无" : refUser.getRegisterMobile());
                    excelVo.setRefRealName(refUser.getRealName());
                    excelVo.setRefMemberLevel(refUser.getMemberLevel());
                    excelVo.setRefLevelName(refUser.getLevelName());
                    ExcelVo upUserPhone = getUpUserPhone(user.getRegisterMobile(), refUser.getRegisterMobile());
                    excelVo.setFinalRefPhone(upUserPhone.getFinalRefPhone());
                    excelVo.setFinalRefName(upUserPhone.getFinalRefName());
                    excelVo.setFinalRefMemberLevel(upUserPhone.getFinalRefMemberLevel());
                    excelVo.setFinalRefLevelName(upUserPhone.getFinalRefLevelName());
                    ExcelVo refPhone = this.getRefPhone(user.getRegisterMobile(), refUser.getRegisterMobile());
                    excelVo.setRecommendPhone(refPhone.getRecommendPhone());
                    excelVo.setRecommendName(refPhone.getRecommendName());
                    excelVo.setRecommendMemberLevel(refPhone.getRecommendMemberLevel());
                    excelVo.setRecommendLevelName(refPhone.getRecommendLevelName());
                }
            }

            switch (user.getMemberLevel()) {
                case 99:
                    excelVo.setCardNum(50);
                    break;
                case 6:
                    if (excelVo.getConsumeAmount() >= 5000) {
                        excelVo.setCardNum(15);
                    }
                    break;
                case 4:
                    if (excelVo.getConsumeAmount() >= 980) {
                        excelVo.setCardNum(5);
                    }
                    break;
            }
    ...
    private ExcelVo getRefPhone(String phone, String refPhone) {
        ExcelVo excelVo = new ExcelVo();
        if (StringUtils.isBlank(refPhone)) {
            excelVo.setRecommendPhone(temp_name);
            excelVo.setRecommendName(temp_name);
            excelVo.setRecommendMemberLevel(temp_level);
            excelVo.setRecommendLevelName(temp_name);
        ...
        User user = this.getUserByPhoneAndMerchantId(phone, "1509688041");
        User refUser = this.getUserByPhoneAndMerchantId(refPhone, "1509688041");
        Integer userLevel = user.getMemberLevel();
        Integer refUserLevel = refUser.getMemberLevel();
        if (refUserLevel > userLevel) {
            excelVo.setRecommendPhone(temp_name);
            excelVo.setRecommendName(temp_name);
            excelVo.setRecommendMemberLevel(temp_level);
            excelVo.setRecommendLevelName(temp_name);
        ...
        excelVo.setRecommendName(refUser.getRealName());
        excelVo.setRecommendLevelName(refUser.getLevelName());
        excelVo.setRecommendMemberLevel(refUser.getMemberLevel());
        excelVo.setRecommendPhone(refUser.getRegisterMobile());
        return excelVo;
    }
     ...
     * @return
     */
    private ExcelVo getUpUserPhone(String phone, String refPhone) {
        ExcelVo excelVo = new ExcelVo();
        if (StringUtils.isBlank(refPhone)) {
            excelVo.setFinalRefPhone(temp_name);
            excelVo.setFinalRefName(temp_name);
        ...
        Integer refUserLevel = refUser.getMemberLevel();
        if (refUserLevel > userLevel) {
            excelVo.setFinalRefLevelName(refUser.getLevelName());
            excelVo.setFinalRefMemberLevel(refUser.getMemberLevel());
            excelVo.setFinalRefName(refUser.getRealName());
            excelVo.setFinalRefPhone(refPhone);
            return excelVo;
        } else {
            if (StringUtils.isBlank(refUser.getReferrer())) {
                excelVo.setFinalRefPhone(temp_name);
                excelVo.setFinalRefName(temp_name);
                excelVo.setFinalRefMemberLevel(temp_level);
                excelVo.setFinalRefLevelName(temp_name);
                ...
                User user1 = this.getByUserId(refUser.getReferrer());
                return this.getUpUserPhone(phone, user1.getRegisterMobile());
            }
        }
